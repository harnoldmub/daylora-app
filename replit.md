...
-   **Guest Invitation Page**: Personalized invitation at `/:slug/guest/:guestId` (`invitation.tsx`). Features RSVP status display (confirmed/pending/declined), table number, party size, dynamic theme colors from `wedding.config.theme`, vertical timeline with segments, locations with Google Maps links, QR code for check-in, cagnotte section, dress code, and "Voir notre site" link. Uses scroll-triggered animations and elegant serif typography.
-   **Email System**: All transactional emails use a shared `emailLayout(wedding, content)` helper in `apps/api/email.ts` for consistent branded design. Includes `esc()` HTML sanitization for user content. Templates: RSVP admin notification, guest confirmation, contribution notification, contributor thank-you, personalized invitation (with confirm/decline buttons), date change apology, premium confirmation. Helper functions `availabilityLabel()` and `availabilityEmoji()` provide human-readable French availability text.
